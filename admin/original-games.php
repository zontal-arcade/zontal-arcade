<?php $page = "Original Games"; ?>
<?php require "../app/includes/function_general.php"; ?>
<?php include "includes/header.php"; ?>
<?php
$search_query = "SELECT * FROM zon_originals ORDER BY id DESC";

if (isset($_GET) && !empty($_GET)) {
    if (isset($_GET['content_type']) && isset($_GET['action']) && isset($_GET['token_id'])) {
        if ($_GET['content_type'] == 'originals') {
            if ($_GET['action'] == 'delete') {
                $id = $_GET['token_id'];
                $sql = "DELETE FROM zon_originals WHERE id=$id";
...
if (isset($_POST) && !empty($_POST)) {
    ...
    foreach($game_id as $id) {
        if(mysqli_query($socket, "INSERT INTO zon_originals (`game_id`) VALUES ($id) ")) {
            echo "<script>window.location.href = '';</script>";
        }
    }
                                ...
                                <tr class="bg-[white] dark:bg-zinc-900 px-16 ">
                                    ...
                                    <td class="text-right relative px-6">
                                        ...
                                        <div id="dc_<?= $row['id'] ?>" style="z-index: 99;"
                                            ...
                                            <a href="?action=delete&token_id=<?= $fg['id'] ?>&content_type=originals"
